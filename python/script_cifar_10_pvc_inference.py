...
def main():
    ...
    if args.optimize == True:
        model = ipex.optimize(model, dtype=opt_dtype)
    ...
    if global_rank == 0:
        logname = f"cifar_10_inference_{args.partition}_nodes_{args.nodes}_gpus_{args.gpus}_batchsize_{args.batch_size}_epochs_{args.epochs}_optimize_{args.optimize}_dtype_{args.dtype}_hierarchy_{args.hierarchy}"
        logging.basicConfig(
