...
def prepare_data(
    ...
    DATA = "/scratch/usr/bzfbnick/pytorch_benchmarking/pytorch-hpc/examples/fit/local_datasets"
...
def main():
    ...
    parser.add_argument(
        "--dataset_root",
        type=str,
        default="/scratch/usr/bzfbnick/intel_bench/pytorch/distributed",
    ...
    device = "xpu:{}".format(mpi_local_rank)
    ...
    if args.optimize == True:
        model, optimizer = ipex.optimize(
            model, optimizer=optimizer, dtype=dtype if args.dtype == "bf16" else None
        )
    ...
    if global_rank == 0:
        logname = f"cifar_10_{args.partition}_nodes_{args.nodes}_gpus_{args.gpus}_batchsize_{args.batch_size}_epochs_{args.epochs}_optimize_{args.optimize}_dtype_{args.dtype}_hierarchy_{args.hierarchy}"
        logging.basicConfig(
